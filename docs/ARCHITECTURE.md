...
- [x] Observability: structured logs, optional Sentry (client + server), sync/transfer metrics
- [x] Server: rate limiting, request size limits, CORS for web origin, graceful shutdown, Dockerfile + `docker compose up` prod profile, backup notes for Postgres/MinIO
- [x] Client: crash-safe DB migrations test (upgrade from every prior schema), storage quota handling, low-disk behavior
- [x] E2E: `integration_test` suite (login → create → sync → conflict → transfer) run in CI on macOS + Android emulator
- [x] Release: app icons/splash, flavors (dev/staging/prod via `--dart-define`), signing docs, CHANGELOG
- [x] Security review checklist (token lifetimes, lock bypass attempts, path traversal in storage keys, chunk-size abuse)
- **Done 2026-09-11.** Notes: server limits are config-driven (`RATE_LIMIT_PER_MINUTE`, `AUTH_RATE_LIMIT_PER_MINUTE`, `MAX_JSON_BODY_BYTES`, `MAX_UPLOAD_BYTES`, `TRUST_PROXY`); the rate limiter is a per-process fixed window keyed by user once authenticated, else by address, and exempts preflights. `/metrics` renders Prometheus text (request counts and latency per normalised route plus sync/upload business counters) only when `METRICS_TOKEN` is set. `LOG_FORMAT=json` swaps in `JsonLogSink`; `SENTRY_DSN` enables Sentry on both sides (the app via `env/*.json` dart-defines, `SentryFlutter.init` wrapping `runApp`). The dart_frog custom `main.dart` handles SIGTERM/SIGINT: stop accepting, wait `SHUTDOWN_GRACE_SECONDS`, close the pool. The image now runs on `debian:bookworm-slim` as a non-root user with CA certs and curl for the health check, and `init` applies pending SQL migrations before serving (the compose drill showed the container came up healthy but with no tables until this was added). The path-traversal test found that `_safe` kept `..` intact; segments equal to `.`/`..` are now neutralised. Client: `AppEnvironment` (dev/staging/prod via `--dart-define-from-file`), Settings › Diagnostics (log buffer from `bootstrap`, sync/transfer state, copy report), Settings › Storage (`CacheManager`: usage, evict uploaded copies), ENOSPC mapped to a plain `StorageFailure` in the importer and a non-retryable transfer failure in downloads. Schema versions are recorded with `scripts/schema.sh` (`drift_schemas/`, generated `test/generated/`) and `migration_test.dart` upgrades from every recorded version and checks data survives. `integration_test/app_e2e_test.dart` runs the real app and encrypted DB against a live API (register → create → sync → conflict from a second device → resolve → 3 MiB upload → download) on macOS and an Android emulator in CI (main and manual runs, since those runners are slow and metered). Release: icons/splash generated from `assets/icon/`, `docs/RELEASE.md`, `docs/OPERATIONS.md` (backups, recovery drills, limits, observability), `docs/SECURITY_REVIEW.md` mapping each control to its test, `CHANGELOG.md`.
